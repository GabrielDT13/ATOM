function buildTree(container, items, isProjectFolder = false) {
    ...
    items.forEach(item => {
        ...
        if (item.type === 'folder') {
            ...
            // 🔹 Icono de estado (play, spinner, check)
            if (isProjectFolder) {
            ...
            // 🔹 Click izquierdo → abrir archivo en el panel central
...
function runProject(projectName, statusIcon) {
    ...
    // 🔹 Cambiar a spinner CSS durante ejecución
    if (statusIcon) {
        statusIcon.src = '';
        statusIcon.classList.add('spinner'); // usa spinner.css
    }
